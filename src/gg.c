...
double * gg_wsum(double x1, double s1, double x2, double s2, int n,
		 double *t1, double *t2, double rho){
  static double ret[3];
...
SEXP gg_rule(SEXP R_x1, SEXP R_s1, SEXP R_x2, SEXP R_s2,
	     SEXP R_t1, SEXP R_t2, SEXP R_rho){
  int n = length(R_x1);
  double *x1, *x2, *t1, *t2;
  x1 = REAL(R_x1);
  x2 = REAL(R_x2);
...
// SURE
SEXP gg_sure(SEXP R_x1, SEXP R_s1, SEXP R_x2, SEXP R_s2,
	     SEXP R_t1, SEXP R_t2, SEXP R_rho){
  int n = length(R_x1);
  double *x1, *x2, *t1, *t2;
  x1 = REAL(R_x1);
...
SEXP gg_min_sure(SEXP R_x1, SEXP R_s1, SEXP R_x2, SEXP R_s2,
  ...
  for(iter = 0; iter < maxit; iter++){
	...
	u = t[j] + C * s2;
      ...
      for(i = 0; i < n; i++){
	tmp = 1 / s1 * phi((x1[i] - t1_new) / s1) / s2 * phi((x2[i] - t2_new) / s2);
	wsum0[i] -= tmp;
